...
export async function main(ns) {
	...
	let moneyThresh = ns.getServerMaxMoney(target)*.07;
...
let gay = true;
	while (gay) {
			...
			if (ns.getServerSecurityLevel(target) > secThresh && ns.hackAnalyzeChance(target) < 1) {
				let weakenPID = 0;
				while (weakenPID == 0) {
	...
	function calcThreads(target, type) {
		let threads;
		const growCap = (ns.getServerMaxRam(ns.getHostname())*.9/ns.getScriptRam('grow.js', 'home'));
		switch (type) {
			case 0:
				if (Math.ceil(ns.hackAnalyzeThreads(target, (moneyThresh*(1-(1/ns.formulas.hacking.growPercent(ns.getServer(target), growCap, ns.getPlayer())))))) > 0) {
					threads = Math.ceil(ns.hackAnalyzeThreads(target, (moneyThresh*(1-(1/ns.formulas.hacking.growPercent(ns.getServer(target), growCap, ns.getPlayer()))))));
				} else {
					threads = 1000;
				}
				break;
			case 1:
					if (moneyThresh/ns.getServerMoneyAvailable(target) == Infinity) {
						threads = growCap;
					} else if (Math.ceil(ns.growthAnalyze(target, (moneyThresh/ns.getServerMoneyAvailable(target)))) > 0) {
